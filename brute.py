...
# Correct TRON derivation using bip_utils
def derive_tron_address(seed_phrase):
    try:
        seed_bytes = Bip39SeedGenerator(seed_phrase).Generate()
        bip44_tron_ctx = Bip44.FromSeed(seed_bytes, Bip44Coins.TRON)
        addr = bip44_tron_ctx.Purpose().Coin().Account(0).Change(Bip44Changes.CHAIN_EXT).AddressIndex(0).PublicKey().ToAddress()
        return addr
    except Exception as e:
        print(f"[ERROR] Tron derivation failed: {e}", flush=True)
        return None
...
def check_trc20_tokentx(address, api_key):
    ...
    try:
        ...
        print(f"[ERROR] Tron API call failed: {e}", flush=True)
    return 0
